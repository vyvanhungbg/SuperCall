...
class ReadableBottomBar @JvmOverloads constructor( context: Context, val attrs: AttributeSet? = null,val  defStyleAttr: Int = 0)
    ...
    private var bottomBarItemList: List<BottomBarItem> = emptyList()
    ...
    fun getAttribute(){
        ...
        itemMenuRes = typedArray.getResourceId(
            R.styleable.ReadableBottomBar_menu,
            itemMenuRes
        )
...
//        val bottomBarItemConfigList = ConfigurationXmlParser(context = context, xmlRes = tabXmlResource!!).parse()
        val bottomBarItemConfigList = BottomBarParser(context = context, res = itemMenuRes).parse()
